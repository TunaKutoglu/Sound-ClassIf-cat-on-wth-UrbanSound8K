...
output_dir = r'C:\Users\tunak\BU BILGISAYRDAKI KODLAR\personal\projeler\Sound classificaiton\SONAR\OUTPUTS'  # Grafiklerin kaydedileceği dizin
...
if X.shape[0] == 0 or y.shape[0] == 0:
    ...
    plt.ylabel('Gerçek')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "karisiklik_matrisi.png"))
    plt.show()
    ...
    for label, (waveform, sr) in sample_waveforms.items():
        ...
        plt.xlabel("Zaman (s)")
        plt.ylabel("Genlik")
        plt.savefig(os.path.join(output_dir, f"dalga_formu_{label}.png"))
        plt.show()
